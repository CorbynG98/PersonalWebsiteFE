...
import { getCookie, removeCookie, setAuthCookie } from '@src/storageclient/storageclient';
...
export const signIn =
  (loginData: LoginData, cancelToken: CancelTokenSource|undefined|null = null): ThunkAction<void, State, unknown, Action> =>
  async (dispatch) => {
    ...
    dispatch({ type: 'SIGN_IN', data: {username: result.username ?? "", token: result.sessionToken ?? ""} });
  };

export const signOut =
  (cancelToken: CancelTokenSource|undefined|null = null): ThunkAction<void, State, unknown, Action> => async (dispatch) => {
