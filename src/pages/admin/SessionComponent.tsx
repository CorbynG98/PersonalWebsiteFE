            ...
            <Table striped bordered hover>
              ...
              <tbody>
                {sessions && sessions.map((session, index) => (
                  <tr key={index}>
                    <td>
                      <p style={{ margin: 0 }}><Moment format="YYYY-MM-DD">{session.createdAt ?? new Date()}</Moment></p>
                    </td>
                    <td>
                    ...
                    <td>
                      <p style={{ margin: 0 }}>{session.latitude}, {session.longitude}</p>
                    </td>
                    <td>
                      {revokingSession == session.id ?
                        ...
                        :
                        <Button onClick={() => showRevokeConfirmAlert(session.id)} disabled={session.currentSession} style={{ backgroundColor: 'transparent', border: 'none', display: 'flex', alignItems: 'center' }}>
                          <FontAwesomeIcon
                            icon={faTrash}
                            size='1x'
                          ...
                          />
                          {session.currentSession ? <p style={{ margin: 0, color: 'black', paddingLeft: '1rem' }}>Current session</p> : <></>}
                        </Button>
                      }
                    </td>
                  </tr>
